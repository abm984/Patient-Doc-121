...
const ClinicalSummaryDisplay: React.FC<ClinicalSummaryDisplayProps> = ({ status, isSummarizing, icdCode }) => {
        ...
        if (status === Status.Idle || status === Status.Active) {
             return (
                <div className="w-full h-full min-h-64 flex items-center justify-center bg-gray-300/30 rounded-lg border-2 border-dashed border-gray-300 p-4">
                    <p className="text-gray-400 text-center">The ICD 10 will appear here after the session ends.</p>
                </div>
            );
        }
        ...
        if (isSummarizing) {
            return (
                <div className="w-full h-full min-h-64 flex flex-col items-center justify-center bg-gray-300/30 rounded-lg border-2 border-dashed border-gray-300 p-4">
                     ...
                     <p className="text-gray-400 mt-4">Generating ICD 10...</p>
                </div>
            );
        }
        ...
        if (icdCode) {
            return (
                <div className="max-w-full overflow-auto bg-gray-200 p-4 rounded-md">
                    <pre className="text-black whitespace-pre-wrap font-sans text-base"><ReactMarkdown>{icdCode}</ReactMarkdown></pre>
                </div>
            );
        }
    ...
    return (
        <div className="w-full max-h-[50vh] overflow-y-auto">
             <h2 className="text-xl font-semibold text-gray-600 mb-4 flex items-center"><SummaryIcon /> ICD 10 Code</h2>
            {renderContent()}
        </div>
    )
};

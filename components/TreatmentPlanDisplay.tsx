...
const ClinicalSummaryDisplay: React.FC<ClinicalSummaryDisplayProps> = ({ status, isSummarizing, treatmentPlan }) => {
        ...
        if (status === Status.Idle || status === Status.Active) {
             return (
                <div className="w-full h-full min-h-64 flex items-center justify-center bg-gray-300/30 rounded-lg border-2 border-dashed border-gray-300 p-4">
                    <p className="text-gray-400 text-center">The Treatment will appear here after the session ends.</p>
                </div>
            );
        }
        ...
        if (isSummarizing) {
            return (
                <div className="w-full h-full min-h-64 flex flex-col items-center justify-center bg-gray-300/30 rounded-lg border-2 border-dashed border-gray-300 p-4">
                     ...
                     <p className="text-gray-400 mt-4">Generating Treatment Plan...</p>
                </div>
            );
        }
    ...
    return (
        <div className="w-full max-h-[50vh] overflow-y-auto">
             <h2 className="text-xl font-semibold text-gray-600 mb-4 flex items-center"><SummaryIcon /> Treatment Plan</h2>
            {renderContent()}
        </div>
    )
};
